...
```{r demo-status, fig.cap="Beruflicher Status der Teilnehmenden"}
# Pie chart was removed in favor for sankey diagram
...
ird <- data %>%
  ...
  mutate(value = factor(value, levels = c(0, 1, 2), labels = c("Nein", "Ja", "Ich weiß nicht"))) %>%
  select(data_id, value)
...
test <- sta %>%
  full_join(lead, by = "data_id") %>%
  count(status, value)

test <- sta %>%
  inner_join(ird, by = "data_id") %>%
  count(status, value)
...
nn_freitext <- freitext %>% distinct(data_id, .keep_all = TRUE) %>% pull(data_id)
```
...
# Chart removed in favor of chart with status group selection
