...
support <- data %>%
  ...
  mutate(label = case_when(
    str_detect(label, "Datenmanagementplan.*") ~ paste0("<span style='font-weight:bold; text-decoration:underline'>", label, "</span>"),
    TRUE ~ label
  )) %>%
  mutate(label = fct_reorder(label, .x = mean, .desc = FALSE))
...
plot_ly(
  ...
  layout_bar_flip(barmode = "stack") %>%
  layout_title(text = "Welchen Bedarf haben Sie an fachlicher Unterstützung\nzu folgenden Themen?") %>%
  layout_caption(hovertext = "M=Mittelwert, Md=Median") %>%
  layout_mode_bar() %>%
  layout(legend = list(orientation = "h")) %>%
...
```{r fehlende-loesungen, fig.cap="Fehlende Lösungen"}
fig <- plot_ly(
  type = "sankey",
...
#seq(0, 0.6, length.out = 8), seq(0.7, 1, length.out = 3)
```
