...
```{r support-thema, fig.cap="Fachliche Unterstützung"}
# Zu welchem der folgenden Themen haben Sie fachliche Unterstützung erhalten?
...
```{r support-erhalten-2, fig.cap="Fachliche Unterstützung und FAIR", out.height="450px"}
support <- data %>%
  filter(str_detect(question_id, "^SUP1$")) %>%
  filter(!value %in% c("-998", "-999")) %>%
  select(data_id,
...
sup_fair %>%
  ...
  add_bars_text() %>%
  layout_bar_flip(barmode = "stack", legend_title = "Kennen o. praktizieren Sie die FAIR-Prinzipien?") %>%
  layout_title(text = "Haben Sie bereits einmal fachliche Unterstützung\ndurch Expert*innen zu Datenmanagement-Themen in Anspruch genommen?") %>%
  layout_caption() %>%
  layout_mode_bar()  
```
...
```{r support-erhalten-3, fig.cap="Fachliche Unterstützung und Transparenz", out.height="450px"}
transparenz <- data %>%
...
sup_transparenz %>%
  ...
  add_bars_text() %>%
  layout_bar_flip(barmode = "stack", legend_title = "Wie beeinflusst Transparenz die FDM-Praxis?") %>%
  layout_title(text = "Haben Sie bereits einmal fachliche Unterstützung\ndurch Expert*innen zu Datenmanagement-Themen in Anspruch genommen?") %>%
  layout_caption() %>%
  layout_mode_bar()
```
